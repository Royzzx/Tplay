...
from tata import download_catchup, download_playback_catchup, trim_video_handler

from utils import check_user, get_tplay_data

from config import api_id, api_hash, bot_token, script_developer, sudo_users, session_string
...
if session_string is not "":
    app = Client("RC_tplay_dl_bot", api_id=api_id, api_hash=api_hash, bot_token=bot_token, session_string=session_string)
else:
...
def start_cmd_handler(app, message):
    message.reply_text("<b>A Telegram Bot</b>`")

@app.on_message(filters.incoming & filters.command(['trim']) & filters.text)
def trim_cmd_handler(app, message):
