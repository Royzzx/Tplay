...
from utils import aria2c, mp4decrpyt, yt_dlp, extract_filename
...
def download_catchup(catchup_url, data_json, app, message):
    ...
    for m in catchup_urls:
        ...
        msg = message.reply_text(f"<b>Processing...</b>")
        tataskyapiurl = f'https://kong-tatasky.videoready.tv/content-detail/pub/api/v1/catchupEpg/{catchup_id}'
...
def trim_video_ffmpeg(input_file, output_file, start_time, end_time, msg):
    ffmpeg_cmd = [
        'ffmpeg',
        '-i', input_file,
        '-ss', start_time,
    ...
    ffmpeg_cmd.extend(["-metadata", f"encoded_by={GROUP_TAG}"])
    ffmpeg_cmd.extend(["-metadata:s:a", f"title={GROUP_TAG}"])
    ffmpeg_cmd.extend(["-metadata:s:v", f"title={GROUP_TAG}"])
...
def trim_video_handler(app, message):
    if message.reply_to_message and message.reply_to_message.video:
        ...
        if len(command) < 2:
            message.reply_text("<b>Syntax: </b>`/trim [start] [end] | [Filename]`")
            return
        
        command = message.text.split("|")[0]
        trim_data = command.split(" ")
        start_time = trim_data[1].strip()
        end_time = trim_data[2].strip()
        req_file_name = message.text.split("|")[-1].strip()
        ...
        msg = message.reply_text(f"<b>Downloading...</b>")

        video_path = app.download_media(video, progress=progress_for_pyrogram, progress_args=("**Downloading...** \n", msg, time.time()))

        print(message.reply_to_message.caption)
        print(extract_filename(message.reply_to_message.caption))

        trimmed_video_path = f'{req_file_name}{extract_filename(message.reply_to_message.caption).split("]")[-1].strip()}'
        print(trimmed_video_path)

        try:
           msg.edit(f"<b>Muxing...</b>")
           filename = trim_video_ffmpeg(video_path, trimmed_video_path, start_time, end_time, msg)
